...
this.L.Control.Geocoder = (function (L) {
  ...
  var OSNamesAPI = L.Class.extend({
    options: {
      serviceUrl: 'https://api.os.uk/search/names/v1/'
    },

    initialize: function(key, geocodeOptions, reverseOptions) {
      this._key = key;
      this._geocodeOptions = geocodeOptions || {};
      this._reverseOptions = reverseOptions || {};
    },

    geocode: function(query, cb, context) {
      var params = JSON.parse(JSON.stringify(this._geocodeOptions));
    ...
    reverse: function(location, scale, cb, context) {
      var params = JSON.parse(JSON.stringify(this._reverseOptions));
      params = Object.assign(
        {
          key: this._key,
          point: location.join(',')
      ...
      getJSON(
        this.options.serviceUrl + 'nearest',
        params,
        L.bind(function(data) {
          cb.call(context, this._resultsHandler(data));
        }, this)
    ...
    _resultsHandler: function(data) {
      ...
      if (data.results && data.results.length) {
        for (var i = 0; i < data.results.length; i++) {
          var result = data.results[i].GAZETTEER_ENTRY;
          ...
          results[i] = {
            name: entity.join(', '),
            html: `<b>${name}</b><br>${entity.slice(1).join(', ')}`,
            center: { ea: result.GEOMETRY_X, no: result.GEOMETRY_Y },
            properties: data.results[i]
          };

          if (result.MBR_XMIN && result.MBR_YMIN && result.MBR_XMAX && result.MBR_YMAX) {
            results[i].bbox = {
              _northEast: { ea: result.MBR_XMAX, no: result.MBR_YMAX },
              _southWest: { ea: result.MBR_XMIN, no: result.MBR_YMIN }
            };
          }
        }
      }
  ...
  function osNamesAPI(key, geocodeOptions, reverseOptions) {
    return new OSNamesAPI(key, geocodeOptions, reverseOptions);
  }

  var OSPlacesAPI = L.Class.extend({
    options: {
      // serviceUrl: 'https://api.ordnancesurvey.co.uk/places/v1/addresses/'
      serviceUrl: 'https://api.os.uk/search/places/v1/'
    },

    initialize: function(key, resource, geocodeOptions, reverseOptions) {
      this._key = key;
      this._resource = resource || 'find';
      this._geocodeOptions = geocodeOptions || {};
      this._reverseOptions = reverseOptions || {};
    },
    ...
    geocode: function(query, cb, context) {
      ...
      var q = this._resource === 'find' ? 'query' : this._resource;
      params[q] = query;

      getJSON(
        this.options.serviceUrl + this._resource,
        params,
        L.bind(function(data) {
          cb.call(context, this._resultsHandler(data));
        }, this)
    ...
    reverse: function(location, scale, cb, context) {
      ...
      params = Object.assign(
        ...
        {
          key: this._key,
          point: [location.lat, location.lng].join(','),
          srs: 'EPSG:4326',
          output_srs: 'EPSG:4326'
        }
      );
    ...
    _resultsHandler: function(data) {
      ...
      if (data.results && data.results.length) {
        for (var i = 0; i < data.results.length; i++) {
          var dataset = data.results[i].hasOwnProperty('DPA') ? 'DPA' : 'LPI';
          var result = data.results[i][dataset];
          ...
          results[i] = {
            name: result.ADDRESS,
            html: `<b>${result.ADDRESS}</b><br>Dataset: ${dataset} | UPRN: ${result.UPRN}`,
            center: latLng,
            bbox: latLngBounds,
            properties: data.results[i]
          };
  ...
  function osPlacesAPI(key, resource, geocodeOptions, reverseOptions) {
    return new OSPlacesAPI(key, resource, geocodeOptions, reverseOptions);
  }
...
L.DomEvent.disableScrollPropagation(this._alts);
